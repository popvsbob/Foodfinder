...
const socket = ref<EventSource | null>(null)
...
const fetchFiles = async () => {
  ...
  try {
    // 相对路径 + Vite 代理：请求由 Vite(5173) 转发给后端(8100)，浏览器无需直连后端端口
...
// 用 EventSource 替代 WebSocket：本项目只需服务端单向推送，
// 且 EventSource 原生支持断线自动重连，无需手写重连逻辑
const connectWebSocket = () => {
  const es = new EventSource(`/api/stream/${currentThreadId.value}`)
...
const handleSocketMessage = (data: any) => {
  ...
  if (event === 'session_created') {
    ...
    if (parts.length > 1) {
      currentSessionUrl.value = `/outputs/${parts[1].replace(/\\/g, '/')}`
    }
...
// Start a New Chat: reset state and reconnect with a fresh thread id
const startNewChat = () => {
  ...
  // Close old SSE connection（必须手动 close，否则 EventSource 会持续自动重连旧会话）
  if (socket.value) {
    socket.value.close()
    socket.value = null
  }
...
<template>
  <div class="app-container">
    <!-- Left Nav Sidebar (DeepSeek style) -->
    <aside class="nav-sidebar">
      <div class="nav-header">
        <div class="nav-logo">✦</div>
        <span class="nav-title">FusionResearch</span>
      </div>
      <button class="new-chat-btn" @click="startNewChat" :disabled="status === 'running'">
        <span class="plus">
      ...
      <div class="nav-footer">FusionResearch · Deep Research</div>
    </aside>
    ...
    <main class="main-content" :class="{ 'centered-layout': isWelcomeScreen }">
      ...
      <div v-if="isWelcomeScreen" class="welcome-screen">
        <div class="welcome-text">
          <h1>Hi，<span class="accent">很高兴见到你</span></h1>
          <h2>我可以帮你搜索资料、深度研究并生成报告，随时告诉我你想了解什么</h2>
        </div>
      </div>
...
<style>
/* Global Resets & Variables (DeepSeek-style light theme) */
:root {
  --bg-dark: #ffffff;          /* main background */
  --surface-dark: #f9fbff;     /* sidebars */
  --surface-light: #f6f7f9;    /* subtle surfaces */
  --text-primary: #262629;
  --text-secondary: #5e5e66;
  --text-tertiary: #a6a6b0;
  --accent-blue: #4d6bfe;      /* DeepSeek brand blue */
  --user-msg-bg: #eff6ff;      /* user bubble light blue */
  --border-color: #e5e6eb;
}
...
.nav-logo {
  ...
  height: 24px;
  border-radius: 6px;
  background: linear-gradient(135deg, #4d6bfe, #7a9bff);
  color: #fff;
  display: flex;
  align-items: center;
